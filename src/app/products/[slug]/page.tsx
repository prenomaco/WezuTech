...
export default async function ProductPage({ params }: ProductPageProps) {
  const { slug } = await params;
  const [product, catalog] = await Promise.all([getProductDetail(slug), getPublishedProducts()]);
  if (!product) notFound();
  const otherProducts = catalog.filter((item) => item.slug !== product.slug);
  return <main className="relative isolate overflow-clip bg-ink"><ProductAtmosphere /><script dangerouslySetInnerHTML={jsonLd(productSchema(product))} type="application/ld+json" /><div className="relative z-10"><ProductDetailPage otherProducts={otherProducts} product={product} /></div></main>;
}
