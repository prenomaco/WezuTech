...
export function AdminProductForm({ product, media }: { product?: Product; media?: ProductMedia | null }) {
  return (
    <details className="admin-card" open={!product}>
      <summary>
        <strong>{product ? `Edit ${product.name}` : "Add product"}</strong>
      </summary>
      <form action={saveProduct} className="admin-form">
        ...
        <label>
          Slug
          <input required name="slug" pattern="[a-z0-9]+(-[a-z0-9]+)*" defaultValue={product?.slug} />
        </label>
        <label>
          Status
        ...
        <label className="wide">
          Card description
          <textarea name="cardDescription" defaultValue={product?.cardDescription ?? ""} rows={3} />
        </label>
        <label className="wide">
          Introduction
          <textarea name="introduction" defaultValue={product?.introduction ?? ""} rows={4} />
        </label>
        <label>
          SEO title
